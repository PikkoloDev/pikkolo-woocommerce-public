...
if (
	...
	function pikkolo_add_vendor_custom_field() {
		...
		echo '<p style="font-weight: bold;">' . __('Pikkoló Custom Fields', 'woocommerce') . '</p>';
	...
	function pikkolo_custom_bulk_edit_fields() {
		?>
		<label>
			<span class="title"><?php _e('Pikkoló: Is frozen', 'woocommerce'); ?></span>
			<select name="_pikkolo_frozen_bulk">
				<option value="no-change"><?php _e('— No change —', 'woocommerce'); ?></option>
				<option value="true"><?php _e('True', 'woocommerce'); ?></option>
				<option value="false"><?php _e('False', 'woocommerce'); ?></option>
			</select>
		</label>
	
		<label>
			<span class="title"><?php _e('Pikkoló: Age restriction', 'woocommerce'); ?></span>
			<select name="_pikkolo_age_restriction_bulk">
				<option value="no-change"><?php _e('— No change —', 'woocommerce'); ?></option>
				<option value="none"><?php _e('None', 'woocommerce'); ?></option>
				<option value="18"><?php _e('18', 'woocommerce'); ?></option>
				<option value="20"><?php _e('20', 'woocommerce'); ?></option>
			</select>
		</label>
		<?php
	}
	...
	function pikkolo_more_info($label, $method) {
		if ($method->id == 'pikkolois') {
			$label .= __(
				'<span data-tooltip="bla" data-flow="top">
					<img src="' . plugins_url('pikkolo/assets/img/info.png', __DIR__) . '" style="max-height:20px;max-width:20px;">
				</span>',
				'pikkolois' 
			);
		}
			...
			if ($order->get_meta('pikkolo_process_error')) {
				add_action(
					...
					printf(
						...
						esc_html($order->get_meta('pikkolo_process_error')
						)
					)
				);
			} else {
